 ...
 * @since 1.0.1
 */
function rapcart_load_plugin_textdomain() {

	$locale = apply_filters( 'plugin_locale', get_locale(), 'rapcart' );

	// Allow upgrade safe, site specific language files in /wp-content/languages/woocommerce/
	load_textdomain( 'rapcart', WP_LANG_DIR . '/woocommerce/rapcart-' . $locale . '.mo' );
	...
	load_plugin_textdomain( 'rapcart', false, basename( dirname( __FILE__ ) ) . '/languages/' );
}

add_action( 'plugins_loaded', 'rapcart_load_plugin_textdomain' );

/**
 * Function that devs can use to check if a page includes the RAPIDCART shortcode
 *
 * @since 1.1
 */
function is_rapcart_checkout( $post_id = null ) {
	...
	if ( empty( $post_id ) || is_admin() ) {
		...
		if ( 0 == FP_One_Page_Checkout::$shortcode_page_id ) {
			$post_to_check = ! empty( $post ) ? $post : get_post( $post_id );
			FP_One_Page_Checkout::check_for_shortcode( $post_to_check );
		}

		// Compare IDs
		if ( $post_id == FP_One_Page_Checkout::$shortcode_page_id || ( 'yes' == get_post_meta( $post_id, '_rapcart', true ) ) ) {
			$is_rapcart = true;
		} else {
			$is_rapcart = false;
		}
	...
	return apply_filters( 'is_rapcart_checkout', $is_rapcart, $post_id );
}

function initialize_one_page_checkout(){
	FP_One_Page_Checkout::init();
}

add_action( 'init', 'initialize_one_page_checkout', -1 );

class FP_One_Page_Checkout {
	private static $nonce_action = 'fp_one_page_checkout';
	...
	static $template = 'checkout/product-list.php';
	...
	public static function init() {
		...
		self::$plugin_url     = untrailingslashit( plugins_url( '/', __DIR__ ) );
		self::$plugin_path    = untrailingslashit( plugin_dir_path( __FILE__ ) );
		self::$template_path  = self::$plugin_path . '/rapcart/templates/';

		require_once self::$plugin_path . '/functions.php';
		// require_once self::$plugin_path . '/classes/class-rapcart-admin-editor.php';
		// require_once self::$plugin_path . '/classes/abstract-class-rapcart-template.php';
		// require_once self::$plugin_path . '/classes/class-rapcart-easy-pricing-tables-template.php';
		// require_once self::$plugin_path . '/classes/class-rapcart-compat-bookings.php';
		// require_once self::$plugin_path . '/classes/class-rapcart-compat-subscriptions.php';
		// require_once self::$plugin_path . '/classes/class-rapcart-compat-name-your-price.php';
		 //require_once self::$plugin_path . '/class-rapcart-settings.php';

		self::$templates   = apply_filters( 'rapcart_templates', array(
			'product-table' => array(
				'label'               => __( 'Product Table', 'rapcart' ),
				'description'         => __( 'Display a row for each product containing its thumbnail, title and price. Best for a few simple products where the thumbnails are helpful, e.g. a set of halloween masks.', 'rapcart' ),
				'supports_containers' => false,
			),
			'product-list' => array(
				'label'               => __( 'Product List', 'rapcart' ),
				'description'         => __( 'Display a list of products with a radio button for selection. Useful when the customer does not need a description or photograph to choose, e.g. versions of an eBook.', 'rapcart' ),
				'supports_containers' => false,
			),
			'funkpd-list' => array(
				'label'               => __( 'Funkpd List', 'rapcart' ),
				'description'         => __( 'Display a list of products with a radio button for selection. Useful when the customer does not need a description or photograph to choose, e.g. versions of an eBook.', 'rapcart' ),
				'supports_containers' => false,
			),
			'product-single'  => array(
				'label'               => __( 'Single Product', 'rapcart' ),
				'description'         => __( "Display the single product template for each product. Useful when the description, images, gallery and other meta data will help the customer choose, e.g. evening gowns.", 'rapcart' ),
				'supports_containers' => false,
			),
			'product-simple'  => array(
				'label'               => __( 'Simple Product', 'rapcart' ),
				'description'         => __( "Display the simple product template for each product. Useful when the description, images, gallery and other meta data will help the customer choose, e.g. evening gowns.", 'rapcart' ),
				'supports_containers' => false,
			),
			'pricing-table'  => array(
				'label'               => __( 'Pricing Table', 'rapcart' ),
				'description'         => __( "Display a simple pricing table with each product's attributes, weight and dimensions. Useful to allow customers to compare different, but related products, e.g. membership subscriptions.", 'rapcart' ),
				'supports_containers' => false,
			),
		) );

		add_filter( 'woocommerce_ajax_get_endpoint', array( __CLASS__, 'make_sure_ajax_url_is_relative' ) );
		add_action( 'rapcart_add_product_selection_fields', array( __CLASS__, 'add_product_selection_fields' ), 9 );

		// Change add to cart messages on RAPIDCART pages to say "Add to Order" and do not include the "View Cart ->" button
		...
		add_shortcode( apply_filters( 'woocommerce_one_page_checkout_shortcode_tag', 'fp_checkout_all_products' ), array( __CLASS__, 'fp_403076_checkout_all_products' ) );
        //add_shortcode( 'fp_checkout_all_products',  array( __CLASS__, 'fp_403076_checkout_all_products') );
		...
		add_filter( 'body_class', array( __CLASS__, 'rapcart_woocommerce_body_class' ) );

		// Filter is_checkout() on RAPIDCART posts/pages
		...
		// Allow empty cart when we're doing a request from a RAPIDCART page.
		...
		// Insert an RAPIDCART specific div for messages/notices
		add_action( 'rapcart_product_selection_fields_before', array( __CLASS__, 'rapcart_messages' ), 10, 2 );

		// Modify RAPIDCART empty cart error
		add_filter( 'woocommerce_add_error', array( __CLASS__, 'improve_empty_cart_error' ) );

		// Load single-product RAPIDCART type-specific templates
		add_action( 'rapcart_single_add_to_cart', array( __CLASS__, 'rapcart_single_add_to_cart' ) );
		add_action( 'rapcart_simple_add_to_cart', array( __CLASS__, 'rapcart_single_add_to_cart_core_types' ) );
		add_action( 'rapcart_variable_add_to_cart', array( __CLASS__, 'rapcart_single_add_to_cart_core_types' ) );
		add_action( 'rapcart_deposit_add_to_cart', array( __CLASS__, 'rapcart_single_add_to_cart_core_types' ) );

		// Unhook 'WC_Form_Handler::add_to_cart_action' from 'init' in RAPIDCART pages, to prevent products from being added to the cart when submitting an order
		if ( isset( $_POST['is_rapcart'] ) && ( ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'woocommerce_checkout' ) || ( isset( $_REQUEST['wc-ajax'] ) && 'checkout' == $_REQUEST['wc-ajax'] ) ) ) {
		...
		// Prepend WC notices to the content if the page is an RAPIDCART page
		add_filter( 'the_content', array( __CLASS__, 'maybe_display_notices' ), 10, 2 );

		// If a link to an RAPIDCART page included the 'add-to-cart' param to automatically add a product to the cart, redirect to the RAPIDCART page without that param (to avoid page refreshes adding the product to the cart, again)
        ...
        //@@
        //marked for deletion
        add_shortcode( 'sc_single_product_rapcart', array( __CLASS__, 'single_product_rapcart' ) );

		add_action( 'woocommerce_after_single_product_summary', array( __CLASS__, 'single_product_rapcart' ), 90 );
		add_action( 'template_redirect', array( __CLASS__, 'filter_single_product_rapcart' ), 30 );

		// Override the checkout template on RAPIDCART pages and Ajax requests to update checkout on RAPIDCART pages
		
		// add_filter( 'wc_get_template', array( __CLASS__, 'override_checkout_template_review' ), 10, 5 );
		// add_filter( 'wc_get_template', array( __CLASS__, 'override_checkout_template_shipping' ), 10, 5 );
		// add_filter( 'wc_get_template', array( __CLASS__, 'override_checkout_template_billing' ), 10, 5 );
		// add_filter( 'wc_get_template', array( __CLASS__, 'override_checkout_template_payment' ), 10, 5 );
		// add_filter( 'wc_get_template', array( __CLASS__, 'override_checkout_template_payment_method' ), 10, 5 );
		// add_filter( 'wc_get_template', array( __CLASS__, 'override_checkout_template_thankyou' ), 10, 5 );
		// add_filter( 'wc_get_template', array( __CLASS__, 'override_checkout_template_cart' ), 10, 5 );
		// add_filter( 'wc_get_template', array( __CLASS__, 'override_checkout_template_cart_totals' ), 10, 5 );
		// add_filter( 'wc_get_template', array( __CLASS__, 'override_checkout_template_orders' ), 10, 5 );
		// add_filter( 'wc_get_template', array( __CLASS__, 'override_checkout_template_order_details_item' ), 10, 5 );

		// Ensure we have a session when loading RAPIDCART pages
		add_action( 'template_redirect', array( __CLASS__, 'maybe_set_session' ), 1 );

		// Enable robots on RAPIDCART pages.
		...
		// Add a variable for rapcart to check what the default/original guest checkout is before extensions like subscriptions override
		if ( self::is_woocommerce_pre( '3.3' ) ) {
			...
			// Make sure the wc_checkout_params.is_checkout JS value is true on custom RAPIDCART pages
		...
		add_action( 'rapcart_before_display_checkout', array( __CLASS__, 'maybe_add_products_to_cart' ) );

		// Initialize our settings.
		//self::get_settings_instance()->init();

		do_action( 'rapcart_loaded' );
	}

	/**
	 * Get the instance of our settings class.
	 *
	 * @return RAPIDCART_Settings
	 */
	// public static function get_settings_instance() {
	// 	static $instance = null;
	// 	if ( null === $instance ) {
	// 		$instance = new RAPIDCART_Settings();
	// 	}
	...
	/**
	 * The master check for an RAPIDCART request. Checks everything from page ID to $_POST data for
	 * some indication that the current request relates to an Ajax request.
	 *
	 * @return bool
	 */
	public static function is_any_form_of_rapcart_page() {

		$is_rapcart = false;

		// Modify template if the page being loaded (non-ajax) is an RAPIDCART page
		if ( is_rapcart_checkout() ) {
		...
		// Modify template when doing a 'woocommerce_update_order_review' ajax request
			...
			if ( isset( $checkout_post_data['is_rapcart'] ) ) {
				$is_rapcart = true;
			}

		// Modify template when doing ajax and sending an RAPIDCART request
	...
	/**
	 * Load custom RAPIDCART order review template to include Remove/Quantity columns.
	 *
	 * @param  boolean $is_ajax
	 * @return void
	 */
	public static function load_rapcart_order_review_template( $deprecated = false ) {
		wc_get_template( 'checkout/deprecated/review-order.php', array( 'checkout' => WC()->checkout(), 'is_ajax' => $deprecated ), '', FP_One_Page_Checkout::$template_path );
	}

	/**
	 * If the cart is empty, and the request is RAPIDCART, disable the `expired session` warning.
	 *
	 * @since 1.7.1
	 */
	public static function maybe_allow_expired_session() {
		if ( WC()->cart->is_empty() && self::is_any_form_of_rapcart_page() ) {
	...
	public static function update_order_review_fragments( $fragments ) {
		...
		if ( self::is_any_form_of_rapcart_page() && 0 == sizeof( WC()->cart->get_cart() ) ) {
	...
	/**
	 * Hook to wc_get_template() and override the checkout template used on RAPIDCART pages and when updating the order review fields
	 * via WC_Ajax::update_order_review()
	 *
	 * @return string
	 */
	public static function override_checkout_template_review( $located, $template_name, $args, $template_path, $default_path ) {
		if ( 'checkout/review-order.php' == $template_name && $default_path !== FP_One_Page_Checkout::$template_path && self::is_any_form_of_rapcart_page() ) {
			
			$located = wc_locate_template( 'checkout/review-order-rapcart.php', '', FP_One_Page_Checkout::$template_path );
		}

		return $located;
	}
	public static function override_checkout_template_shipping( $located, $template_name, $args, $template_path, $default_path ) {
		if ( 'checkout/form-shipping.php' == $template_name && $default_path !== FP_One_Page_Checkout::$template_path && self::is_any_form_of_rapcart_page() ) {
			$located = wc_locate_template( 'checkout/form-shipping.php', '', FP_One_Page_Checkout::$template_path );
		}

		return $located;
	}
	public static function override_checkout_template_billing( $located, $template_name, $args, $template_path, $default_path ) {
		if ( 'checkout/form-billing.php' == $template_name && $default_path !== FP_One_Page_Checkout::$template_path && self::is_any_form_of_rapcart_page() ) {
			$located = wc_locate_template( 'checkout/form-billing.php', '', FP_One_Page_Checkout::$template_path );
		}

		return $located;
	}	
	public static function override_checkout_template_payment( $located, $template_name, $args, $template_path, $default_path ) {
		if ( 'checkout/payment.php' == $template_name && $default_path !== FP_One_Page_Checkout::$template_path && self::is_any_form_of_rapcart_page() ) {
			$located = wc_locate_template( 'checkout/payment.php', '', FP_One_Page_Checkout::$template_path );
		}

		return $located;
	}
	public static function override_checkout_template_payment_method( $located, $template_name, $args, $template_path, $default_path ) {
		if ( 'checkout/payment-method.php' == $template_name && $default_path !== FP_One_Page_Checkout::$template_path && self::is_any_form_of_rapcart_page() ) {
			$located = wc_locate_template( 'checkout/payment-method.php', '', FP_One_Page_Checkout::$template_path );
		}

		return $located;
	}
	public static function override_checkout_template_thankyou( $located, $template_name, $args, $template_path, $default_path ) {
		if ( 'checkout/thankyou.php' == $template_name && $default_path !== FP_One_Page_Checkout::$template_path ) {
			$located = wc_locate_template( 'checkout/thankyou.php', '', FP_One_Page_Checkout::$template_path );
		}
		return $located;
	}
	public static function override_checkout_template_cart( $located, $template_name, $args, $template_path, $default_path ) {
		if ( 'cart/cart.php' == $template_name && $default_path !== FP_One_Page_Checkout::$template_path ) {
			$located = wc_locate_template( 'cart/cart.php', '', FP_One_Page_Checkout::$template_path );
		}
		return $located;
	}
	public static function override_checkout_template_cart_totals( $located, $template_name, $args, $template_path, $default_path ) {
		if ( 'cart/cart-totals.php' == $template_name && $default_path !== FP_One_Page_Checkout::$template_path ) {
			$located = wc_locate_template( 'cart/cart-totals.php', '', FP_One_Page_Checkout::$template_path );
		}
		return $located;
	}
	public static function override_checkout_template_orders( $located, $template_name, $args, $template_path, $default_path ) {
		if ( 'myaccount/orders.php' == $template_name && $default_path !== FP_One_Page_Checkout::$template_path ) {
			$located = wc_locate_template( 'myaccount/orders.php', '', FP_One_Page_Checkout::$template_path );
		}
		return $located;
	}
	public static function override_checkout_template_order_details_item( $located, $template_name, $args, $template_path, $default_path ) {
		if ( 'order/order-details-item.php' == $template_name && $default_path !== FP_One_Page_Checkout::$template_path ) {
			$located = wc_locate_template( 'order/order-details-item.php', '', FP_One_Page_Checkout::$template_path );
		}
		return $located;
	}

	/**
	 * RAPIDCART single-product template action for custom product types - templates not included with RAPIDCART (must be loaded externally by hooking at this point).
	 *
	 * @param  int $post_id
	 * @return void
	 */
	public static function rapcart_single_add_to_cart( $post_id ) {
		global $product;

		$product_type = rapcart_get_product_type( $product );
		...
		do_action( 'rapcart_' . $product_type . '_add_to_cart', $post_id );

		remove_filter( 'woocommerce_product_single_add_to_cart_text', array( __CLASS__, 'modify_single_add_to_cart_text' ) );
	}

	/**
	 * RAPIDCART single-product template action for core product types - templates are included with RAPIDCART and loaded directly.
	 *
	 * @param  int $post_id
	 * @return void
	 */
	public static function rapcart_single_add_to_cart_core_types( $post_id ) {
		global $product;
		wc_get_template( 'checkout/add-to-cart/' . rapcart_get_product_type( $product ) . '.php', array( 'product' => $product ), '', FP_One_Page_Checkout::$template_path );
	}

	/**
	 * If we're on a RAPIDCART page, filter the default WC add to cart message to say "added to order".
	 *
	 * @since 1.1
	 */
	...
	/**
	 * If we're on a RAPIDCART page, filter the default WC error messages to remove the view cart button.
	 *
	 * @since 1.1.1
	 */
	public static function maybe_filter_error_message( $message ) {
		...
		if ( is_rapcart_checkout() ) {
			...
			$message = preg_replace('/<a[^>]*>(' . __( 'View Cart', 'rapcart' ) .')<\/a>/iU','',$message);
	...
	protected static function get_add_to_cart_message( $quantity, $product_title ) {
		...
		return sprintf( __( '%s&quot; added to your order. Complete your order below.', 'rapcart' ), $product_title );
	}

	/**
	 * Change button 'Add to cart' text to 'Add to order' in RAPIDCART pages
	 * @param  WC_Product $product
	 * @return string
	 */
	public static function modify_single_add_to_cart_text( $product ) {
		return __( 'Add to order', 'rapcart' );
	}
	...
	public static function add_product_selection_fields() {
		...
		do_action( 'rapcart_product_selection_fields_before', self::$template, self::$raw_shortcode_atts );

		if ( false === apply_filters( 'rapcart_show_product_selection_fields', true, self::$template ) ) {
			return;
		}
		...
		if ( ! empty( self::$products_to_display ) || ! empty( self::$categories_to_display ) ) {
			...
			$args = apply_filters( 'rapcart_products_query_args', $args );
		...
		$products = apply_filters( 'rapcart_products_for_selection_fields', $products, self::$template, self::$raw_shortcode_atts );

		?>
		<div id="rapcart-product-selection" data-rapcart_id="<?php echo self::$shortcode_page_id; ?>" class="fpc-container">
			<?php if ( ! empty( $products ) ) : ?>
				<?php wc_get_template( self::$template, array( 'products' => $products ), '', self::$template_path ); ?>
			<?php endif; ?>
		</div><!-- .rapcart-product-selection --><?php

		self::maybe_show_shipping( $products );

		do_action( 'rapcart_product_selection_fields_after', self::$template, self::$raw_shortcode_atts );
	}

	/**
	 * Used to generate data that maps RAPIDCART list/table-template products to cart items.
	 *
	 * @param  WC_Product $product
	...
	public static function ajax_remove_from_cart() {
		...
		do_action( 'rapcart_ajax_remove_from_cart_response_before' );
		...
		foreach ( $cart as $cart_item_id => $value ) {
			...
			// Requests coming from the RAPIDCART order-review template reference a specific cart item by its key.
			...
			// Requests coming from RAPIDCART items reference their own product_id and RAPIDCART id.
			...
			wc_add_notice( sprintf( __( '&quot;%s&quot; was successfully removed from your order.', 'rapcart' ), get_the_title( $value['product_id'] ) ), 'success' );
		...
		if ( ! $item_removed ) {
			wc_add_notice( sprintf( __( '&quot;%s&quot; could not be removed from your order.', 'rapcart' ), get_the_title( $value['product_id'] ) ), 'error' );
		...
		$response_data = apply_filters( 'rapcart_ajax_remove_from_cart_response_data', $response_data );

		WC()->cart->maybe_set_cart_cookies();

		echo json_encode( $response_data );

		do_action( 'rapcart_ajax_remove_from_cart_response_after' );
	...
	public static function ajax_add_to_cart( $bypass = false ) {
		...
		if ( isset( $_REQUEST['empty_cart'] ) && ! apply_filters( 'rapcart_not_empty_cart', false ) ) {
			WC()->cart->empty_cart();
		}
		...
		if ( ! $bypass ) {
			...
			if ( 'variation' === $add_to_cart_handler ) {
				...
				$product_id         = rapcart_get_variation_parent_id( $product );
				...
				foreach ( $variations as $name => $value ) {
					...
					if ( $value ) {
						...
						if ( ! taxonomy_exists( esc_attr( str_replace( 'attribute_', '', $name ) ) ) ) {
							$variations[ $name ] = FP_One_Page_Checkout::get_formatted_attribute_value( $name, $value, $attributes );
						}
						continue;
					}
				...
				if ( $all_variations_set ) {
					...
					wc_add_notice( __( 'Please choose product options&hellip;', 'rapcart' ), 'error' );
				}
			...
			// Custom Handler
			} elseif ( has_action( 'rapcart_add_to_cart_handler_' . $add_to_cart_handler ) ){

				do_action( 'rapcart_add_to_cart_handler_' . $add_to_cart_handler, $url );
			...
			// Simple Products
				...
				if ( $passed_validation ) {
					...
					if ( WC()->cart->add_to_cart( $product_id, $quantity ) ) {
						wc_add_notice( self::get_add_to_cart_message( $quantity, rapcart_get_products_name( $product ) ), 'success' );
						$was_added_to_cart = true;
					}
				}
		...
		do_action( 'rapcart_ajax_add_to_cart_response_before' );
		...
		$response_data = apply_filters( 'rapcart_ajax_add_to_cart_response_data', $response_data );

		echo json_encode( $response_data );

		do_action( 'rapcart_ajax_add_to_cart_response_after' );
	...
	public static function enqueue_scripts() {
		...
		wp_enqueue_script( 'woocommerce-one-page-checkout', self::$plugin_url . '/assets/js/one-page-checkout.js', array( 'jquery', 'wc-add-to-cart-variation' ), '1.0', true );

		$params = apply_filters( 'rapcart_script_data', array(
			'rapcart_nonce'                 => wp_create_nonce( self::$nonce_action ),
			'rapcart_complete_order_prompt' => '<a class="wc-south rapcart-complete-order" href="#customer_details">' . __( 'Modify &amp; complete order below', 'rapcart' ) . '</a>',
			'ajax_error_notice'           => '<div class="woocommerce-error">' . __( 'Error processing your request. Please try refreshing the page. Contact us if you continue to have issues.', 'rapcart' ) . '</div>',
			'ajax_url'                    => WC()->ajax_url(),
		//	'autoscroll'                  => self::get_settings_instance()->get_setting( 'autoscroll' ),
		) );

		wp_localize_script( 'woocommerce-one-page-checkout', 'rapcart', $params );
		...
		do_action( 'rapcart_enqueue_scripts' );
	}

	/**
	 * Enqueue the WooCommerce single product styles & scripts if the page is using an RAPIDCART
	 ...
	 * let's check the current post/posts to see if any contain the RAPIDCART shortcode.
	 *
	 * @since 1.0
	 */
	public static function ensure_shortcode_page_id_is_set( $posts, $query ) {
		...
		if ( 0 == self::$shortcode_page_id ) {
			foreach ( $posts as $post ) {
				if ( ( false !== stripos( $post->post_content, '[woocommerce_one_page_checkout' ) ) || ( 'yes' == get_post_meta( $post->ID, '_rapcart', true ) ) ) {
	...
	public static function is_checkout_hack( $page_id ) {
		...
		if ( 0 != self::$shortcode_page_id ) {
			...
			$functions_to_ignore = apply_filters( 'rapcart_is_checkout_override_function_names', array( 'wc_template_redirect', 'get_checkout_url', 'get_checkout_payment_url', 'get_checkout_order_received_url', 'get_cancel_order_url', 'get_cancel_order_url_raw', 'is_checkout' ) );

			// An array of backtrace indexes to look for functions to ignore
			$backtrace_indexes = apply_filters( 'rapcart_is_checkout_override_function_backtrace_indexes', array( 3, 4, 5, 6 ) );
	...
	/**
	 * Filter the result of `is_checkout()` for RAPIDCART posts/pages
	 *
	...
	public static function check_for_shortcode( $post_to_check ) {
		...
		||  false !== stripos( $post_to_check->post_content, '[fp_checkout_all_products' ) ) {
			self::$add_scripts = true;
	...
	/**
	 * Evaluate the RAPIDCART shortcode
	 *
	 * @since 1.0
	 */
	public static function get_one_page_checkout( $atts ) {
		...
		return WC_Shortcodes::shortcode_wrapper( __CLASS__ . '::one_page_checkout_shortcode', $atts, array( 'id'  => 'fp_checkout', 'class'  => 'fp_checkout', 'before' => null, 'after'  => null ) );
	


	}
        
    public static function fp_403076_checkout_all_products() 
    {
		// don't evaluate shortcode more than once on the same page
		if ( true === self::$evaluated_shortcode || is_admin() ) {
			return '';
		...
		$atts = array('template' => 'product-list','product_ids' => self::fp_937952_echo_product_ids());

		return WC_Shortcodes::shortcode_wrapper( __CLASS__ . '::one_page_checkout_shortcode', $atts, array( 'id'  => 'fp_checkout', 'class'  => 'fp_checkout', 'before' => null, 'after'  => null ) );
        
    }
    
    public static function fp_937952_echo_product_ids() 
    {
        $all_ids = '';
        $out = '';
	...
	public static function one_page_checkout_shortcode( $atts ){
		...
		if ( ! rapcart_is_frontend_request() ) {
			return;
		}
        ...
        // VERSION 2 WORKING - TOGGLE WITH V1 TO CHANGE
        if ( ! is_admin() ) 
        {
        ...
        // VERSION 1 WORKING - TOGGLE WITH V2 TO CHANGE
        // if ( ! is_admin() ) {

        //     echo "aaa " . $atts['product_ids'] ;
        //     echo "<br> " ;
        //     print_r(explode(",", $atts['product_ids']));
        ...
        //         {
        //             $cart_item_ids[] .= $values['product_id'];
        //         } 
        ...
        //     $non_in_cart = array_diff($product_ids,$cart_item_ids);

        //     WC()->cart->add_to_cart( implode($non_in_cart) );
		...
		if ( isset( $atts['template'] ) && ! empty( $atts['template'] ) ) {
			...
			if ( file_exists( wc_locate_template( $atts['template'] . '.php', self::$template_path ) ) ) {

                self::$template = $atts['template'] . ".php";
			...
			self::$template = apply_filters( 'rapcart_template', self::$template, $atts );
		}
		...
		do_action( 'rapcart_before_display_checkout' );
		...
		wc_get_template( 'checkout/form-checkout.php', array( 'checkout' => $checkout ), '', FP_One_Page_Checkout::$template_path );
	...
	public static function improve_empty_cart_error( $error ) {
		...
		if ( defined( 'WOOCOMMERCE_CHECKOUT' ) && $error == sprintf( __( 'Sorry, your session has expired. <a href="%s">Return to homepage &rarr;</a>', 'rapcart' ), home_url() ) ) {
			$error = __( 'You must select a product.', 'rapcart' );
		}
	 ...
	 * To allow table/list templates to recognize & manage their own cart items, pass the id of the current RAPIDCART container to retrieve cart items added by this RAPIDCART container only.
	 *
	 * @param  int   $rapcart_id  Only return cart items managed by a specific RAPIDCART page.
	 * @return array          Associated array of with product or variation IDs as the keys and quantity as the values.
	 * @since 1.0
	 */
	private static function get_products_in_cart( $rapcart_id = false ) {
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( false === apply_filters( 'rapcart_allow_cart_item_modification', true, $cart_item, $cart_item_key, $rapcart_id ) ) {
				continue;
			}
		...
		if ( current_user_can( 'activate_plugins' ) ) :
			if ( ! is_woocommerce_active() ) : ?>
				<div id="message" class="error">
					<p><?php printf( __( '%sWooCommerce One Page Checkout is inactive.%s The %sWooCommerce plugin%s must be active for WooCommerce One Page Checkout to work. Please %sinstall & activate WooCommerce%s', 'rapcart' ), '<strong>', '</strong>', '<a href="http://wordpress.org/extend/plugins/woocommerce/">', '</a>', '<a href="' . admin_url( 'plugins.php' ) . '">', '&nbsp;&raquo;</a>' ); ?></p>
				</div>
						<?php elseif ( version_compare( get_option( 'woocommerce_db_version' ), '2.5', '<' ) ) : ?>
				<div id="message" class="error">
					<p><?php printf( __( '%sWooCommerce One Page Checkout is inactive.%s This plugin requires WooCommerce 2.5 or newer. Please %supdate WooCommerce to version 2.5 or newer%s', 'rapcart' ), '<strong>', '</strong>', '<a href="' . admin_url( 'plugins.php' ) . '">', '&nbsp;&raquo;</a>' ); ?></p>
				</div>
		<?php endif; ?>
	<?php endif;
	}
	 ...
	 */
	public static function rapcart_woocommerce_body_class($classes) {
		global $post;
		...
		if ( 'yes' == get_post_meta( $post->ID, '_rapcart', true ) ) {
			array_push($classes, 'rapcart-product-single' );
		}
	...
	/**
	 * If we're on an RAPIDCART page, display any WC notices at the top of the page - useful in case the store manager
	 * used a customer add-to-cart link and we want to display the success/error message at the top of the RAPIDCART page
	 * after the refresh.
	 *
	 * @since 1.1
	 */
	public static function maybe_display_notices( $content ) {
		...
		if ( rapcart_is_frontend_request() && is_rapcart_checkout() ) {
			ob_start();
			wc_print_notices();
	...
	/**
	 * Insert an RAPIDCART specific div for messages/notices. Helps with determining whether messages are displayed within
	 * the viewport or not and allows better targeting of RAPIDCART specific messages/notices.
	 *
	 * @since 1.1
	 */
	public static function rapcart_messages( $template, $raw_shortcode_atts ) {

		echo '<div id="rapcart-messages" class="rapcart-messages"><div class="woocommerce-message" role="alert">Add or remove something to update your cart</div></div>';

	}

	/**
	 * If the store manager has manually added an add-to-cart param to the RAPIDCART page ID, after adding the product
	 * to the cart, redirect to the RAPIDCART page without the add-to-cart param, to avoid adding the product again if
	...
	public static function add_to_cart_redirect( $url ) {
		...
		if ( ! is_ajax() && is_rapcart_checkout() ) {
			$schema = is_ssl() ? 'https://' : 'http://';
			$url = explode('?', $schema . $_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"] );
	...
	public static function product_type_options( $options ){
		...
		$options['rapcart'] = array(
			'id'            => '_rapcart',
			'wrapper_class' => '',
			'label'         => __( 'One Page Checkout', 'rapcart'),
			'description'   => __( 'Add checkout to product page.', 'rapcart'),
			'default'       => 'no'
		);
	...
	public static function save_product_meta( $post_id, $post ) {
		...
		if ( isset( $_POST['_rapcart'] ) ) {
			update_post_meta( $post_id, '_rapcart', 'yes' );
		} else {
			update_post_meta( $post_id, '_rapcart', 'no' );
		}

	}

	/**
	 * Append rapcart checkout form template to core single product template if enabled
	 */
	public static function single_product_rapcart() {
		if ( is_rapcart_checkout() && is_product() ) {
	...
	/**
	 * Modifications to the core single product section when rapcart is enabled
	 */
	public static function filter_single_product_rapcart() {
		if ( is_rapcart_checkout() ) {
			...
			if ( is_product() ) {
				...
				if ( ! empty( $product ) && 'yes' == rapcart_get_products_prop( $product, 'rapcart', '_' ) ) {
					if ( $product->is_type( 'variable' ) || $product->is_type( 'grouped' ) ) {
						$products = array_filter( array_map( 'wc_get_product',
							rapcart_get_all_child_products( $product ) ) );
					} else {
			...
			if ( is_page() ) {
				// Removes tabs when we're using a page as RAPIDCART.
	...
	/**
	 * Make sure a session is set whenever loading an RAPIDCART page.
	 *
	 ...
	 * 1. loading an RAPIDCART page when they were not logged in an had no ideas in the cart, there would be no session
	 *    so the nonce set on the page at the time of load would not use the session's customer_id
	 * 2. once the customer added an item to the cart from the RAPIDCART page, the session would be set, but then the
	...
	public static function maybe_set_session() {
		if ( is_woocommerce_active() ) {
			if ( is_rapcart_checkout() && ! WC()->session->has_session() ) {
	...
	/**
	 * Re-enables robots on RAPIDCART pages.
	 *
	...
	/**
	 * Make the un modified guest checkout option value available to rapcart and ensure the woocommerce js attaches to the create account checkbox (we'll hide it using js)
	 */
	public static function filter_woocommerce_script_paramaters( $woocommerce_params, $handle = '' ) {
		...
		if ( self::is_any_form_of_rapcart_page() ) {
			$woocommerce_params['rapcart_option_guest_checkout'] = get_option( 'woocommerce_enable_guest_checkout' );
	...
	/**
	 * Conditionally load custom RAPIDCART order review template to include Remove/Quantity columns.
	 * Must be done only when viewing an RAPIDCART page.
	 *
	 * @return void
	 * @deprecated 1.7.0
	 */
	public static function rapcart_order_review_template_actions() {
		wc_deprecated_function( __METHOD__, '1.7.0' );
	}
}
