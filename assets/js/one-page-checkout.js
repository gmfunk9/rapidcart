...
jQuery(document).ready(function ($) {
    ...
    // Quantity buttons
    $checkout.on('change input', '#order_review .rapcart_cart_item div.quantity input.qty', function (e) {

        var input = $(this),
            selectors = '.checkout #order_review .rapcart_cart_item div.quantity input.qty';
        ...
        timeout = setTimeout(function () {
            ...
            var data = {
                quantity: input.val(),
                add_to_cart: parseInt(input.closest('.rapcart_cart_item').data('add_to_cart')),
                update_key: input.closest('.rapcart_cart_item').data('update_key'),
                nonce: rapcart.rapcart_nonce,
            };
    ...
    // Remove buttons
    $checkout.on('click', '#order_review .rapcart_cart_item a.remove', function (e) {

        var data = {
            action: 'pp_remove_from_cart',
            add_to_cart: parseInt($(this).closest('.rapcart_cart_item').data('add_to_cart')),
            update_key: $(this).closest('.rapcart_cart_item').data('update_key'),
            nonce: rapcart.rapcart_nonce,
        },
            selectors = '.checkout #order_review .rapcart_cart_item a.remove';
    ...
    $('#rapcart-product-selection button.single_add_to_cart_button, .rapcart-product-single button.single_add_to_cart_button').on('click', function (e) {
        ...
        var is_variable = $(this).closest('.variations_form').find('input[name="variation_id"]').length === 1,
            ...
            data = {
                ...
                nonce: rapcart.rapcart_nonce,
                input_data: $(this).closest('.product-quantity, .rapcart-product-single form').find('input[name!="variation_id"][name!="product_id"][name!="add-to-cart"][name!="quantity"], select, textarea').serialize(),
            },
            selectors = '#rapcart-product-selection button.single_add_to_cart_button, .rapcart-product-single button.single_add_to_cart_button';
    ...
    $('#rapcart-product-selection input[type="number"][data-add_to_cart]').on('change input', function (e) {
        var input = $(this),
            selectors = '#rapcart-product-selection input[type="number"][data-add_to_cart]';
        ...
        timeout = setTimeout(function () {
            ...
            if (data['quantity'] >= e.currentTarget.max ) {
                data['quantity'] = e.currentTarget.max;
            }
            if (data['quantity'] == 0) {
                data['action'] = 'pp_remove_from_cart';
            } else {
    ...
    $('#rapcart-product-selection input[type="radio"][data-add_to_cart], #rapcart-product-selection input[type="checkbox"][data-add_to_cart]').on('change', function (e) {

        var input = $(this),
            selectors = '#rapcart-product-selection input[type="radio"][data-add_to_cart], #rapcart-product-selection input[type="checkbox"][data-add_to_cart]';
        ...
        timeout = setTimeout(function () {
            ...
            if (input.is(':checked')) {
                ...
                input.parents('.fpc-product ').addClass('selected');

            } else {

                data.action = 'pp_remove_from_cart';
                input.parents('.fpc-product ').removeClass('selected');
    ...
    /* Add/remove products with button element or a tags */
    $('#rapcart-product-selection a[data-add_to_cart], #rapcart-product-selection button[data-add_to_cart]').on('click', function (e) {
        ...
        var data = {
            ...
            selectors = '#rapcart-product-selection a[data-add_to_cart], #rapcart-product-selection button[data-add_to_cart]';
    ...
    $body.on('updated_checkout', function () {
        if (response_messages.length > 0) {
            var $rapcart_messages = $('#rapcart-messages');

            $rapcart_messages.prepend(response_messages);

            if ('yes' === rapcart.autoscroll && !$rapcart_messages.visible()) {
                $('html, body').animate({
                    scrollTop: ($rapcart_messages.offset().top - 150)
                }, 500);
            }
    ...
    $.fn.ajax_add_remove_product = function (data, e, selectors, async_ajax) {
        ...
        $('body').trigger('rapcart_add_remove_product', [data, e, selectors]);

        // Read from rapcart_add_remove_product trigger above and maybe avoid the AJAX add to cart call.
        ...
        $.ajax({
            ...
            success: function (response, status, xhr) {
                ...
                try {
                    ...
                    var inputs = $('#rapcart-product-selection [data-add_to_cart]');
                    ...
                    if ('undefined' == typeof response.messages) {
                        response_messages = rapcart.ajax_error_notice;
                    } else {
                        response_messages = response.messages;
                    }

                    var $rapcart_messages = $('#rapcart-messages');
                    $rapcart_messages.prepend(response.messages);

                    $('html, body').animate({
                        scrollTop: ($rapcart_messages.offset().top - 50)
                    }, 500);
                }

                $('#rapcart-messages .woocommerce-error, #rapcart-messages .woocommerce-message, #rapcart-messages .woocommerce-info').remove();
                ...
                $body.trigger('after_rapcart_add_remove_product', [data, response]);
    ...
    function initOrderReviewQtyButtons() {
        ...
        $('#order_review.rapcart_order_review div.quantity:not(.buttons_added)').addClass('buttons_added').append('<input type="button" value="+" class="plus" />').prepend('<input type="button" value="-" class="minus" />');
    }
    ...
    function initSingleProductTemplateButtons() {
        ...
        $('#rapcart-product-selection button.single_add_to_cart_button').each(function () {

            $(this).after(rapcart.rapcart_complete_order_prompt);
    ...
    function create_account_toggle() {
        ...
        if ($('.rapcart_order_review').length) {
            ...
            if ($('.rapcart_order_review tfoot .recurring-totals').children().length > 0) {
                ...
                if ('no' === wc_checkout_params.rapcart_option_guest_checkout) {
                    $p_create_account.hide();
